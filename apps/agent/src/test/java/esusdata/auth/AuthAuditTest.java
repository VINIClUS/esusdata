    ...
    void grantMutationIsAuditedWithoutSecretOrClinicalData() throws Exception {
        ...
        List<Map<String, Object>> rows = jdbc.queryForList(
                "select event_type, actor_user_id, target, detail_json from auth_audit"
                        + " where target = ? order by at",
                target);
        assertThat(rows)
                .anyMatch(row -> "GRANT_ADDED".equals(row.get("event_type")) && admin.equals(row.get("actor_user_id")));
    }

    @Test
    void deniedAccessIsAuditedWithoutClinicalData() throws Exception {
        String outsider = createUser("outsider-" + System.nanoTime());
        HttpResponse<String> denied = HttpClient.newHttpClient()
                .send(
                        HttpRequest.newBuilder(URI.create(BASE_URL + "/api/v1/results?municipalityIbge=" + MUNICIPALITY
                                ...
                                .header("Cookie", sessionCookie(outsider))
                                .GET()
                                .build(),
        ...
        List<Map<String, Object>> rows = jdbc.queryForList(
                "select event_type, outcome, detail_json from auth_audit"
                        + " where actor_user_id = ? and event_type = 'ACCESS_DENIED'",
                outsider);
