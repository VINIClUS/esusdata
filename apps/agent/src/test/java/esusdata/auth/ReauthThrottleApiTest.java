...
public class ReauthThrottleApiTest extends ApiFixtureSupport {
    ...
    void wrongReauthenticationIsThrottledByAccountAndOrigin() throws Exception {
        ...
        for (int attempt = 0; attempt < 5; attempt++) {
            HttpResponse<String> failed = authenticatedPost(
                    cookie, URI.create(BASE_URL + "/api/v1/auth/reauth"), "{\"password\":\"wrong-reauth-password\"}");
            assertThat(failed.statusCode()).isEqualTo(401);
            assertThat(failed.body()).contains("AUTHENTICATION_FAILED");
        }

        HttpResponse<String> throttled = authenticatedPost(
                cookie, URI.create(BASE_URL + "/api/v1/auth/reauth"), "{\"password\":\"wrong-reauth-password\"}");
        ...
        List<Map<String, Object>> reauthAudit = jdbc.queryForList(
                "select outcome from auth_audit" + " where actor_user_id = ? and event_type = 'REAUTH' and target = ?",
                userId,
                userId);
        ...
        List<Map<String, Object>> attempts =
                jdbc.queryForList("select outcome from login_attempts where username = ?", userId);
