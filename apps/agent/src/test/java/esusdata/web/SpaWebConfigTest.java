...
public class SpaWebConfigTest extends SecuritySliceTestSupport {
    ...
    void rootServesIndexRevalidatedAndWithTheWebClientHeaders() throws Exception {
        ...
        assertThat(root.headers().firstValue("Content-Type")).hasValueSatisfying(
                type -> assertThat(type).startsWith("text/html"));
        assertThat(root.headers().firstValue("Cache-Control")).hasValue("no-cache");
        assertThat(root.headers().firstValue("Content-Security-Policy")).hasValueSatisfying(
    ...
    void hashedAssetsAreCachedForeverAndMissingFilesAre404() throws Exception {
        ...
        assertThat(asset.headers().firstValue("Cache-Control")).hasValueSatisfying(
    ...
    void apiPathsStayOnTheApiChainAndNeverFallBackToIndex() throws Exception {
        ...
        assertThat(ready.headers().firstValue("Cache-Control")).hasValueSatisfying(
                cache -> assertThat(cache).contains("no-store"));
