...
public class RuntimeScopeApiTest extends ApiFixtureSupport {
    ...
    void periodsAreDistinctNewestFirstAndScoped() throws Exception {
        ...
        HttpResponse<String> outOfScope = get(manager, "/api/v1/results/periods?municipalityIbge=" + OTHER_MUNICIPALITY);
    ...
    void runsListRejectsOutOfScopeAndOutOfRangeRequests() throws Exception {
        ...
        assertThat(get(auditor, "/api/v1/runs?municipalityIbge=" + RUNS_MUNICIPALITY).statusCode()).isEqualTo(404);
        assertThat(get(manager, "/api/v1/runs?municipalityIbge=" + RUNS_MUNICIPALITY + "&limit=0").statusCode())
                .isEqualTo(400);
        assertThat(get(manager, "/api/v1/runs?municipalityIbge=" + RUNS_MUNICIPALITY + "&limit=101").statusCode())
                .isEqualTo(400);
    }

    private void publish(String manager, String referencePeriod) throws Exception {
        publishResult(manager, MUNICIPALITY, referencePeriod, new IndicatorResult(
                ...
                referencePeriod, "c1-mais-acesso@0.1.0", referencePeriod + "-28", MUNICIPALITY, List.of(),
                "c1-exact-ratio@1"), List.of());
    }
    ...
    private void insertJob(String jobId, String municipalityIbge, Instant createdAt) {
        jdbc.update("""
                ...
                VALUES (?,?,?,?,?,?, 'QUEUED', 0, 3, 0, ?)
                """, jobId, "run-" + jobId, municipalityIbge, "c1-mais-acesso", "c1-mais-acesso@0.1.0",
                "2026-08", createdAt.toString());
    }
    ...
    private HttpResponse<String> get(String userId, String path) throws Exception {
        return HttpClient.newHttpClient().send(
                HttpRequest.newBuilder(URI.create(BASE_URL + path))
                        .header("Cookie", sessionCookie(userId)).GET().build(),
