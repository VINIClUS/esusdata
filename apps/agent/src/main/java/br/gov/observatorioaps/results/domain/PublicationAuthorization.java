 ...
 * authorization is still current, without depending on {@code access} — {@code results} must
 * stay readable/testable with the PEC disconnected and, by the same reasoning, without the
 * identity capability wired in (an invariant {@code ModuleBoundaryTest} enforces). Implemented by
 * {@code access.application.GrantRevalidator}.
 *
